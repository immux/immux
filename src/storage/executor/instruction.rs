    ...
    fn to_string(&self) -> String {
        match self {
            ...
            Instruction::InspectOne { key } => {
                format!("Instruction InspectOne, key: {:?}", key.to_string())
            }
            Instruction::InspectAll => format!("Instruction InspectAll"),
            Instruction::Insert { key, content } => format!(
                "Instruction Insert, key: {:?}, content: {:?}",
                key.to_string(),
                content
            ),
            Instruction::RevertOne { key, height } => format!(
                "Instruction RevertOne, key: {:?}, height: {:?}",
                key.to_string(),
                height
            ),
            Instruction::RevertAll { height } => {
                format!("Instruction RevertAll, height: {:?}", height)
            }
            Instruction::RemoveOne { key } => {
                format!("Instruction RemoveOne, key: {:?}", key.to_string())
            }
            ...
            Instruction::TransactionCommit { transaction_id } => format!(
                "Instruction TransactionCommit, transaction_id {:?}",
                transaction_id
            ),
            Instruction::TransactionAbort { transaction_id } => format!(
